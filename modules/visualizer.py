...
# --- HELPER 1: INTERPOLACIÓN OPTIMIZADA (Basado en V2 - Fuente 592-608) ---
def run_interp(df_puntos, metodo, bounds_box):
    try:
        # OPTIMIZACIÓN: Resolución 75j para equilibrio entre velocidad y detalle
        gx, gy = np.mgrid[
...
# --- HELPER 2: PROMEDIOS REALES (Basado en V2 - Fuente 612) ---
def calcular_promedios_reales(df_datos):
    if df_datos.empty:
        return pd.DataFrame()
    
    # Filtro: Contamos registros válidos
    conteo = (
        df_datos[df_datos[Config.PRECIPITATION_COL] >= 0]
        .groupby([Config.STATION_NAME_COL, Config.YEAR_COL])
        .size()
    )
    # OPTIMIZACIÓN: Exigir al menos 5 años (menos estricto que V1 que pedía 10)
...
# --- HELPER 3: VECTORIZACIÓN E ISOYETAS (V2 - Fuente 639) ---
def generate_isohyets_gdf(gx, gy, gz, levels=15, crs="EPSG:4326"):
    ...
    try:
        ...
        for collection in contour.collections:
            ...
            try:
                z_val = collection.get_label() # Intento obtener etiqueta
                if not isinstance(z_val, (int, float)):
                     # Fallback si matplotlib no etiqueta directo
                    z_val = 0 
            except:
                z_val = 0
            ...
            for path in paths:
                if len(path.vertices) >= 2:
                    ...
                    values.append(z_val) # Nota: Aquí se podría mejorar la extracción exacta del nivel Z
                    
        if not lines:
            return None
            
        return gpd.GeoDataFrame({"precip_mm": values, "geometry": lines}, crs=crs)
    except Exception as e:
        print(f"Error vectorizando: {e}")
        return None

# --- HELPER 4: ZIP SHAPEFILE (V2 - Fuente 669) ---
...
def display_advanced_maps_tab(df_long, gdf_stations, **kwargs):
    ...
    st.subheader(f"🗺️ Superficies de Interpolación{titulo_meses} y Análisis Hidrológico")
    ...
    # Obtener ubicación del usuario (Manejo de error de V1 corregido con V2)
    ...
    if mode == "Regional (Comparación)":
        ...
        with c1:
            ...
            r1 = st.slider("Rango P1:", 1980, 2025, (1990, 2000), key="r1")
            m1 = st.selectbox("Método P1:", ["Kriging (RBF)", "IDW (Lineal)", "Spline"], key="m1")
        with c2:
            st.markdown("###### Periodo 2 (Reciente)")
            r2 = st.slider("Rango P2:", 1980, 2025, (2010, 2020), key="r2")
        ...
        if st.session_state.get("regional_done"):
            ...
            # Sub-función interna para plotear paneles (V1/V2 Logic)
                ...
                if len(df_m) > 2:
                    ...
                    if gz is not None:
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            marker=dict(color="black", size=5, line=dict(width=1, color="white")),
                        ...
                        if u_loc:
                            fig.add_trace(go.Scatter(
                                x=[u_loc[1]], y=[u_loc[0]], mode="markers+text",
                        ...
                        # Mapa Folium en Expander
                        with col.expander(f"🔎 Ver Mapa Detallado ({tag})", expanded=False):
                            center_lat = (bounds[2] + bounds[3]) / 2
                            center_lon = (bounds[0] + bounds[1]) / 2
                            ...
                            for _, row in df_m.iterrows():
                                ...
                                folium.CircleMarker(
                                    [row["latitude"], row["longitude"]], radius=5, color="blue", fill=True,
                                    fill_color="cyan", fill_opacity=0.7, tooltip=html
                                ).add_to(m)
                            st_folium(m, height=300, use_container_width=True, key=f"folium_{tag}")

            plot_panel(p["r1"], p["m1"], c1, "A", user_loc)
            plot_panel(p["r2"], p["m2"], c2, "B", user_loc)

    # -------------------------------------------------------------------------
    # MODO 2: POR CUENCA (DETALLADO - INTEGRACIÓN V1 + V2)
    # -------------------------------------------------------------------------
    else:
        ...
        if sel_cuencas:
            ...
            rng_c = c_p1.slider("Periodo Análisis:", 1980, 2025, (2000, 2020))
            ...
            if st.button("🚀 Analizar Cuenca"):
                ...
                with st.spinner("Procesando geometría, hidrología e indicadores..."):
                    
                    # A. GEOMETRÍA OPTIMIZADA (V2)
                    sub = gdf_subcuencas[gdf_subcuencas[col_name].isin(sel_cuencas)]
                    
                    # Simplificación para gráficos rápidos (Source: 843)
                    try:
                        geom_union_shape = sub.geometry.simplify(0.01).unary_union
                        # GDF Real para cálculos morfológicos precisos
                    ...
                    # Buffer
                    buf_shape = geom_union_shape.buffer(buffer_km / 111.32) # Aprox grados
                    gdf_buffer = gpd.GeoDataFrame({"geometry": [buf_shape], "tipo": "Buffer"}, crs=gdf_stations.crs)

                    # B. FILTRADO ESPACIAL Y TEMPORAL
                    ...
                    if not stns_zone.empty:
                        ...
                        if len(df_interp) >= 3:
                            ...
                            try:
                                ...
                                st.error(f"Error FDC: {e}")

                            # F. GUARDAR RESULTADOS EN SESSION_STATE (Crucial para persistencia)
                            st.session_state["basin_res"] = {
                                "ready": True, "gz": gz, "gx": gx, "gy": gy,
                                "gdf_puntos": gdf_puntos_interp, "gdf_cuenca": gdf_union_real,
                            ...
                            st.error("Insuficientes estaciones (<3) en el área de influencia.")
            ...
            if res and res.get("ready"):
                ...
                # Puntos
                df_p = res["gdf_puntos"]
                fig.add_trace(go.Scatter(x=df_p.geometry.x, y=df_p.geometry.y, mode="markers+text", 
                                        marker=dict(color="red", size=6), text=res["df_interp"][Config.STATION_NAME_COL],
                                        textposition="top center", name="Estaciones"))
                
                if user_loc:
                     fig.add_trace(go.Scatter(x=[user_loc[1]], y=[user_loc[0]], mode="markers", marker=dict(color="black", symbol="star", size=12), name="TÚ"))

                fig.update_layout(height=500, margin=dict(l=0, r=0, b=0, t=30))
                st.plotly_chart(fig, use_container_width=True)

                # 2. DESCARGAS GIS (Funcionalidad V2)
                st.markdown("---")
                st.subheader("💾 Descarga de Capas GIS")
                c_gis1, c_gis2 = st.columns(2)
                
                gdf_iso = res.get("gdf_isoyetas")
                gdf_basin = res["gdf_cuenca"]

                # Botones simples GeoJSON
                c_gis1.download_button("📥 Cuenca (GeoJSON)", data=gdf_basin.to_json(), file_name="cuenca.geojson", mime="application/json")
                if gdf_iso is not None:
                    c_gis2.download_button("📥 Isoyetas (GeoJSON)", data=gdf_iso.to_json(), file_name="isoyetas.geojson", mime="application/json")
                ...
                with st.expander("📦 Descargar como Shapefile (.zip)"):
                    ...
                    try:
                        if gdf_iso is not None:
                            ...
                            s1.download_button("Descargar Isoyetas (.shp)", zip_iso, "isoyetas.zip", "application/zip")
                        zip_basin = create_zipped_shapefile(gdf_basin, "cuenca")
                        s2.download_button("Descargar Cuenca (.shp)", zip_basin, "cuenca.zip", "application/zip")
                    except Exception as e:
                        st.warning(f"No se pudieron generar los Shapefiles: {e}")

                # 3. MÉTRICAS Y BALANCE (Texto V1 Restaurado)
                st.markdown("---")
                ...
                with st.expander("ℹ️ Metodología: Balance Hídrico de Turc (Detalle)"):
                    st.markdown(r"""
                ...
                # 4. ÍNDICES CLIMÁTICOS (Texto V1 Restaurado)
                st.markdown("---")
                ...
                # 5. CURVA DURACIÓN DE CAUDALES (FDC)
                fdc_data = res.get("fdc")
                if fdc_data:
                    st.markdown("---")
                    st.subheader("📉 Curva de Duración de Caudales (FDC)")
                    
                    # Manejo flexible dict/df
                    if isinstance(fdc_data, dict):
                        df_fdc = fdc_data.get("data")
                        eq_fdc = fdc_data.get("equation", "N/A")
                        r2_fdc = fdc_data.get("r_squared", 0)
                    else:
                        df_fdc, eq_fdc, r2_fdc = fdc_data, "N/A", 0
                    ...
                    if df_fdc is not None and not df_fdc.empty:
                        ...
                        with st.expander("ℹ️ ¿Qué es la Curva de Duración?"):
                            st.markdown("""
                            La **Curva de Duración de Caudales (FDC)** muestra el porcentaje de tiempo que un caudal es igualado o excedido.
                            * **Q95 (95%):** Caudal ecológico o mínimo confiable.
                            * **Q50 (50%):** Caudal mediano.
                            Fundamental para diseño de bocatomas y concesiones.
                            """)

                # 6. CURVA HIPSOMÉTRICA (Texto V1 Restaurado - VITAL)
                # Verifica si existe la función en el módulo analysis
                if hasattr(analysis, "calculate_hypsometric_curve"):
                    try:
                        ...
                        if hyp:
                            ...
                            with st.expander("ℹ️ Interpretación Hipsométrica (Estado Evolutivo)"):
                                st.markdown("""
                                La curvatura indica la "edad" geomorfológica de la cuenca:
                                * **Curva Convexa (Juventud):** La línea se abomba hacia arriba. Cuenca inestable, alto potencial erosivo. (Integral > 0.60).
                                * **Curva en "S" (Madurez):** La línea cruza el centro. Equilibrio entre erosión y sedimentación. (Integral 0.35 - 0.60).
                                * **Curva Cóncava (Vejez):** La línea se hunde. Cuenca estabilizada, sedimentación predominante. (Integral < 0.35).
                                
                                *Bibliografía: Strahler, A. N. (1952).*
                                """)
                    except Exception as e:
                        pass # Silencioso si falla hipsometría

                # 7. MAPA CONTEXTO FOLIUM
                st.markdown("---")
                st.subheader("🗺️ Contexto Espacial")
                if "bounds" in res:
                    bnd = res["bounds"]
                    m_ctx = folium.Map([(bnd[2]+bnd[3])/2, (bnd[0]+bnd[1])/2], zoom_start=10, tiles="CartoDB positron")
                    
                    if res.get("gdf_buffer") is not None:
                        folium.GeoJson(res["gdf_buffer"], style_function=lambda x: {"color": "gray", "dashArray": "5,5", "fill": False}, name="Area Influencia").add_to(m_ctx)
                    
                    folium.GeoJson(res["gdf_cuenca"], style_function=lambda x: {"color": "blue", "weight": 2, "fillOpacity": 0.1}).add_to(m_ctx)
                    
                    # Popups detallados (V1/V2 style)
                    ...
                    if df_raw_ctx is not None and df_interp_ctx is not None:
                        for _, row in df_interp_ctx.iterrows():
                            ...
                            folium.CircleMarker(
                                [row["latitude"], row["longitude"]], radius=6, color="darkred", fill=True,
                                fill_color="red", fill_opacity=0.9, popup=popup, tooltip=f"{nm}"
                            ).add_to(m_ctx)
