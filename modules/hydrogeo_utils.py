...
def calcular_balance_turc(df_lluvia, altitud, ki):
    """Calcula Turc. Si hay NaNs en lluvia, el resultado será NaN (correcto en hidrología)."""
    df = df_lluvia.copy()
    ...
    if 'valor' in df.columns: col_p = 'valor' 

    df['ds'] = pd.to_datetime(df[col_fecha])
    
    # Resample mantiene NaNs si el mes no tiene datos
    ...
    # Operaciones vectorizadas (propagan NaNs automáticamente)
...
# ==============================================================================
# 3. CÁLCULO ESTADÍSTICO (Respetando NaNs)
# ==============================================================================
@st.cache_data(show_spinner=False, ttl=600)
def obtener_estadisticas_estaciones(_engine, df_puntos_snapshot):
    """
    Calcula estadísticas.
    IMPORTANTE: No rellena con ceros. Si no hay dato, queda NaN.
    """
    ...
    # Inicializar columnas con NaN (no con 0.0)
    ...
    try:
        with _engine.connect() as conn:
            ...
            # SQL AVG ignora nulos automáticamente, lo cual es correcto.
            q_text = f"""
    ...
    # Merge manteniendo NaNs donde no hubo cruce
    df_res = pd.merge(df_res, df_stats, left_on='id_estacion', right_on='id_estacion', how='left')

    # Cálculo Vectorial (Manejo de NaNs)
    # Si p_men es NaN, todo lo demás será NaN automáticamente
    ...
    # np.where es seguro con NaNs, pero si denom es NaN, etr será NaN
