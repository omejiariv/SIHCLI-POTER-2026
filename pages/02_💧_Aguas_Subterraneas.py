...
if gdf_zona is not None:
    ...
    if df_res.empty:
        st.error("Error en el cálculo del modelo. Verifica si hay suficientes datos históricos.")
        st.stop()
    ...
    # KPIs
    # Ahora es seguro acceder a 'tipo' porque validamos que no esté vacío
    ...
    with tab2:
        ...
        if 'suelos' in layers: folium.GeoJson(layers['suelos'], name="Suelos", style_function=lambda x: {'color': 'green', 'weight': 0.5, 'fillOpacity': 0.1}).add_to(m)
        if 'hidro' in layers: folium.GeoJson(layers['hidro'], name="Hidrogeología", style_function=lambda x: {'color': 'blue', 'weight': 0.5, 'fillOpacity': 0.1}).add_to(m)
        if 'bocatomas' in layers: folium.GeoJson(layers['bocatomas'], name="Bocatomas", marker=folium.CircleMarker(radius=3, color='red')).add_to(m)
