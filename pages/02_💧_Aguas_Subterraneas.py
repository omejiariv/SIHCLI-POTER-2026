...
with st.sidebar:
    ...
    usar_forecast = st.checkbox("Activar Proyección IA", value=True)
    
    meses_futuros = 12
    if usar_forecast:
        st.info("El modelo detectará dónde terminan los datos reales y llenará el hueco hasta hoy + el futuro.")
...
if ids_seleccionados:
    ...
    if not df_precip.empty:
        ...
        with tab1:
            ...
            # 1. Agrupación mensual y LIMPIEZA DE DATOS FANTASMA
            df_ts_monthly = df_full.groupby('fecha')['valor'].mean().reset_index()
            
            # --- GUILLOTINA TEMPORAL ---
            # Eliminamos nulos y ceros al final de la serie si parecen ser errores de registro
            df_ts_monthly = df_ts_monthly.dropna(subset=['valor'])
            
            # Detectamos la última fecha REAL con datos
            if not df_ts_monthly.empty:
                last_hist_date = df_ts_monthly['fecha'].max()
                
                # Opcional: Si el último valor es 0, verificar si es "cero real" o "falta de dato".
                # Para seguridad, asumimos que la serie histórica llega hasta el último registro en BD.
                
                df_final_ts = df_ts_monthly.copy()
                df_final_ts['tipo'] = 'Histórico'

                # 2. MOTOR DE PRONÓSTICO (PROPHET - PUENTE)
                if usar_forecast and len(df_ts_monthly) > 24: # Necesitamos historia para entrenar
                    with st.spinner(f"🧠 Entrenando IA desde {last_hist_date.date()}..."):
                        try:
                            ...
                            # CÁLCULO DEL HUECO (GAP) + FUTURO
                            # Desde el último dato histórico hasta HOY + FUTURO
                            fecha_objetivo = datetime.now() + relativedelta(months=meses_futuros)
                            
                            # Generar dataframe de fechas futuras
                            future = m.make_future_dataframe(periods=300, freq='MS') # Generamos de más y recortamos
                            future = future[future['ds'] <= fecha_objetivo]
                            
                            forecast = m.predict(future)
                            
                            # Filtrar solo lo que es estrictamente nuevo (después del último dato real)
                            df_future = forecast[forecast['ds'] > last_hist_date][['ds', 'yhat']].rename(columns={'ds': 'fecha', 'yhat': 'valor'})
                            df_future['tipo'] = 'Pronóstico'
                            df_future['valor'] = df_future['valor'].clip(lower=0) # No lluvia negativa
                            
                            # Concatenar
                            df_final_ts = pd.concat([df_final_ts, df_future])
                            
                            # Mensaje de éxito
                            gap_months = (datetime.now().year - last_hist_date.year) * 12 + (datetime.now().month - last_hist_date.month)
                            st.success(f"✅ Serie completada: Se llenó un hueco de {gap_months} meses y se proyectó hasta {fecha_objetivo.date()}.")
                ...
                # Ojo: Si un año tiene mezcla de Histórico y Pronóstico, lo etiquetamos como "Mixto" o tomamos el dominante.
                # Para simplificar gráficos, sumamos valores.
                ...
                st.warning("No hay datos válidos en la selección.")
